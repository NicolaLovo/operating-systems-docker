// Da https://github.com/koszio/Quicksort-with-Pthreads/blob/master/quickSort.c
...
int main(int argc, char **argv){
    ...
    int thrN[] = {1, 2, 4, 6, 8, 10, 12, 20, 50, 100};    
	...
	initArray[i] = (int)(rand()%999);
    }

    for(int k=0;k<10;k++){	
	    int depthOfThreadCreation = thrN[k];  
	    
	    int *arrayElements = malloc(size*  sizeof(int));

	    for (int i=0 ; i<size ; i++){
		arrayElements[i] = initArray[i];
	    }  
	    ...
	    printf("\n");
            printf("The execution time using %d thread was:  %lf secs\n", depthOfThreadCreation, end_time - start_time);
